...
load(here("Data/dens_biomass_mono.RData"))
names(dens)
...
info_communities <- read_csv(here("Data", "info_communities.csv"))
...
# read complete_aggr
empirical_results <- read.csv(here("Data", "empirical_results.csv"))
...
summary(lm(log10(CV) ~ log10(cv_com), data = empirical_results_summary_stats1))
